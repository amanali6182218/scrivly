  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50/60 to-orange-50/40">
      ...
      <header className="border-b border-amber-100 bg-white/80 backdrop-blur-sm">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3 sm:px-6">
          <span className="flex items-center gap-2 text-base font-bold text-stone-900 sm:text-lg">
            ...
            <img src="/logo.png" alt="Scrivly" style={{ height: "44px", width: "auto" }} />
            Scrivly
          </span>
          <div className="flex items-center gap-3">
            <Link
              href="/login"
              className="rounded-lg border border-stone-200 bg-white px-4 py-2 text-sm font-medium
                text-stone-700 shadow-sm transition hover:bg-stone-50"
            >
              Sign in
            </Link>
            <Link
      ...
      <main className="mx-auto max-w-4xl px-4 py-20 text-center sm:px-6 sm:py-28">
        <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-amber-200 bg-amber-50 px-4 py-1.5 text-xs font-semibold text-amber-700">
        ...
        <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
          <Link
            href="/signup"
            className="w-full rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 px-8 py-3.5
              text-base font-semibold text-white shadow-md shadow-amber-500/30 transition
              hover:from-amber-600 hover:to-orange-600 sm:w-auto"
          >
            Start for free — 10 credits
          </Link>
          <Link
            href="/login"
            className="w-full rounded-xl border border-stone-200 bg-white px-8 py-3.5 text-base
              font-medium text-stone-700 shadow-sm transition hover:bg-stone-50 sm:w-auto"
          >
        ...
        {/* Social proof bar */}
        <div className="mt-10 rounded-full bg-amber-100/70 px-4 py-2.5 text-xs font-medium text-amber-800 sm:text-sm">
        ...
        <div className="mt-16 grid gap-6 text-left sm:grid-cols-3">
          {FEATURES.map((feature, i) => (
            <div
              key={feature.title}
              className="animate-slide-up rounded-2xl border border-amber-100 bg-white p-6 shadow-sm"
              style={{ animationDelay: `${i * 100}ms` }}
            >
              <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-xl bg-amber-50">
                {feature.icon}
              </div>
              <h3 className="mb-2 font-semibold text-stone-900">{feature.title}</h3>
        ...
        <div className="mt-20">
          ...
          <div className="mt-10 grid gap-6 rounded-2xl border border-amber-100 bg-white/70 p-6 text-left shadow-sm sm:grid-cols-2 sm:p-8">
            ...
            <div className="space-y-4">
              <div>
                <p className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-amber-700">Listing title</p>
              ...
              <div>
                <p className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-amber-700">Tags</p>
              ...
              <div className="flex flex-wrap gap-2">
                <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-100 px-3 py-1 text-xs font-medium text-emerald-700 ring-1 ring-inset ring-emerald-200">
                  $28 – $45 · High demand
                </span>
                <span className="inline-flex items-center gap-1.5 rounded-full bg-amber-100 px-3 py-1 text-xs font-medium text-amber-700 ring-1 ring-inset ring-amber-200">
                  Score: 91/100 ✓
                </span>
              </div>
            </div>
            ...
            {PRICING_PLANS.map((plan) => (
              <div
                ...
                className={`relative rounded-2xl border bg-white p-6 shadow-sm ${
                  plan.featured ? "border-amber-300 ring-2 ring-amber-200" : "border-amber-100"
                }`}
              >
                {plan.featured && (
                  <span className="absolute -top-3 right-6 rounded-full bg-gradient-to-r from-amber-500 to-orange-500 px-3 py-1 text-xs font-semibold text-white shadow-sm">
                    Most Popular
                  </span>
                )}
                <h3 className="font-semibold text-stone-900">{plan.name}</h3>
                <p className="mt-2 text-3xl font-extrabold text-stone-900">
                  {plan.price} <span className="text-sm font-medium text-stone-400">one-time</span>
                </p>
                <p className="mt-3 text-sm text-stone-600">{plan.credits}</p>
                <p className="text-sm text-stone-500">{plan.listings}</p>
                <p className="mt-2 text-sm font-medium text-amber-700">{plan.blurb}</p>
                <a
                  href="#"
      ...
      <footer className="border-t border-amber-100 bg-amber-50/60">
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-4 px-4 py-6 text-center text-xs text-stone-500 sm:flex-row sm:px-6 sm:text-left">
          {/* eslint-disable-next-line @next/next/no-img-element */}
